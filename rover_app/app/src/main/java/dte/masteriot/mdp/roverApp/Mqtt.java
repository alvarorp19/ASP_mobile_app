...
public class Mqtt implements Runnable{
    ...
    String serverHost = "srv-iot.diatel.upm.es";  // Replace with your Mosquitto broker's IP if different
    int serverPort = 8883;

    // Topics
    private static String sendInfoToRover = "v1/devices/me/telemetry";   //topic from which the mobile APP will publish info
    private static String  receivedInfoFromRover= "v1/devices/me/attributes"; //topic from which the mobile APP will be suscribed
    ...
    void createMQTTclient() {
        client = MqttClient.builder()
                ...
                .username("c80zn9tfv9oiluyp2tss") // Configurar el nombre de usuario
                .password("".getBytes(StandardCharsets.UTF_8)) // Contraseña vacía como arreglo de bytes
                .build()
                )
    ...
    private static void subscribeToTopics() {
        client.subscribeWith()
                .topicFilter(receivedInfoFromRover)
                .callback(publish -> {
...
//                    msg.sendToTarget();
                ...
                .whenComplete((subAck, throwable) -> {
                    if (throwable != null) {
                        Log.d(TAG, "Problem subscribing to MQTT topic:");
                        Log.d(TAG, throwable.toString());
                    } else {
    ...
    public static void publishStopRequest() {

        String message = "{prueba: 12345}";

        Log.d(TAG,"Publishing at " + sendInfoToRover);

        client.publishWith()
                .topic(sendInfoToRover)
